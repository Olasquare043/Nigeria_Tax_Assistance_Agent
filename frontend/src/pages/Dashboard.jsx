...
function Dashboard() {
  // IMPORTANT: Add clearRecentChats to the destructuring
  const {
    messages,
    loading,
    error,
    ...
    startNewChat,
    loadChat,
    clearRecentChats, // <-- ADD THIS LINE
    setError
  } = useChat(); // <-- Make sure this hook has clearRecentChats
  ...
  // Debug logging - Add clearRecentChats to logs
  useEffect(() => {
        ...
        {showSidebar && (
          <div className="fixed lg:relative inset-y-0 left-0 z-30 lg:z-auto w-64 lg:w-72 h-full bg-surface border-r border-border lg:block shadow-lg lg:shadow-none">
            <Sidebar
              ...
              onDisclaimerClick={() => setShowDisclaimerModal(true)}
              onFAQsClick={() => setShowFAQsModal(true)}
              onClearRecentChats={clearRecentChats} // <-- ADD THIS LINE
              user={user}
              onProfileClick={() => navigate('/profile')}
              onLogout={handleLogout}
            />

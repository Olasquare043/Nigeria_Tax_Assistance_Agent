...
def route_node(state: TaxState) -> TaxState:
    ...
    # Extract just the current user text for pattern matching
...
def retrieve_node(state: TaxState) -> TaxState:
    ...
    # Use context-aware query for better retrieval
    user_text_with_context = _get_user_message_with_context(state, include_context=True)
...
def answer_node(state: TaxState) -> TaxState:
    ...
    # Get context-aware user message
    user_text_with_context = _get_user_message_with_context(state, include_context=True)
    current_user_text = _last_user_text(state)  # For citation building
    retrieved = state.get("retrieved", []) or []
    route = state.get("route", "qa")
    ...
    if route == "claim_check":
        ...
        prompt = CLAIM_CHECK_PROMPT.format(
            user_message=user_text_with_context,  # Now includes context
            claim=current_user_text.strip(),
            verdict=verdict,
            evidence_quotes=evidence_quotes,
        )
    ...
    # -------------------------
    if not citations:
        # Use context in refusal message
